...
const CHAPTERS = [
  ...
  },
  {
    ...
    title: "Planet",
    lead:
      "Miyawaki plantation programmes across Delhi/NCR, led by Sustainability Head Mrs. Abha Batra, alongside lower-impact fabric sourcing.",
    theme: "dark" as const,
    stats: [
      ...
      <main className="bg-cream">
        <section className="pt-28 sm:pt-32 pb-14 sm:pb-16 px-5 sm:px-8 text-center">
          ...
          <p className="mt-4 max-w-lg mx-auto text-stone text-base sm:text-lg leading-relaxed">
            Sustainability is an exercise in both social and environmental well-being — a
            commitment to people, the planet, and responsible manufacturing.
          </p>
        </section>
        ...
        {CHAPTERS.map((chapter, i) => {
          ...
          return (
            <section
              key={chapter.id}
              id={chapter.id}
              className={`scroll-mt-24 relative overflow-hidden border-t border-sand ${
                dark ? "bg-ink" : i % 2 === 1 ? "bg-white" : ""
              }`}
            >
              <span
                aria-hidden
                className={`pointer-events-none absolute -top-6 sm:-top-10 right-4 sm:right-8 font-display font-bold text-[7rem] sm:text-[14rem] leading-none tracking-tight select-none ${
                  dark ? "text-white/5" : "text-ink/[0.04]"
                }`}
              >
                {chapter.number}
              </span>
              ...
              <div className="relative mx-auto max-w-6xl px-5 sm:px-8 py-16 sm:py-24">
                <SectionReveal>
                  <p className={`text-xs uppercase tracking-[0.2em] ${dark ? "text-champagne" : "text-champagne"}`}>
                    Chapter {chapter.number}
                  </p>
                  <TextReveal
                    as="h2"
                    text={chapter.title}
                    className={`mt-2 font-display font-semibold text-4xl sm:text-6xl tracking-tight ${
                      dark ? "text-white" : "text-ink"
                    }`}
                  />
                  <p className={`mt-5 max-w-xl text-sm sm:text-base leading-relaxed ${dark ? "text-white/60" : "text-stone"}`}>
                    {chapter.lead}
                  </p>
                </SectionReveal>
                ...
                <div className="mt-12 grid grid-cols-2 lg:grid-cols-4 gap-4">
                  {chapter.stats.map((stat, j) => (
                    <SectionReveal key={stat.value} delay={j * 0.06}>
                      <div className={dark ? "card bg-white/[0.04] border-white/10 p-5 h-full" : "card p-5 h-full"}>
                        <p className={`font-display text-2xl sm:text-3xl font-bold leading-none tracking-tight ${dark ? "text-champagne" : "text-champagne"}`}>
                          {stat.value}
                        </p>
                        <p className={`mt-3 text-xs sm:text-sm leading-relaxed ${dark ? "text-white/60" : "text-taupe"}`}>
                          {stat.label}
                        </p>
                      </div>
                    </SectionReveal>
