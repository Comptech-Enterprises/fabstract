    ...
    <section className="py-32 bg-bone">
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
        <motion.div
          ...
          className="mb-16"
        >
          <p className="text-olive-drab text-sm tracking-[0.3em] uppercase mb-4">FAQ</p>
          <h2 className="text-smoky-black text-3xl sm:text-4xl font-bold">Frequently Asked Questions</h2>
        </motion.div>
        <div className="divide-y divide-smoky-black/10">
          {faqs.map((faq, i) => (
...
function FAQItem({ q, a, i }: { q: string; a: string; i: number }) {
  ...
  return (
    ...
    >
      ...
      >
        <span className="text-smoky-black font-semibold text-base sm:text-lg group-hover:text-olive-drab transition-colors">{q}</span>
        <span className={`flex-shrink-0 w-8 h-8 rounded-full border border-smoky-black/15 flex items-center justify-center transition-transform duration-300 ${open ? "rotate-45" : ""}`}>
          <svg className="w-4 h-4 text-smoky-black/50" fill="none" viewBox="0 0 24 24" stroke="currentColor">
            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 4v16m8-8H4" />
          </svg>
        </span>
      </button>
      {open && (
        <p className="pb-7 text-smoky-black/60 text-base leading-relaxed max-w-4xl">{a}</p>
      )}
    </motion.div>
  );
}
